...
import { createSupportSession } from "@/modules/platform-admin/service";
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    return NextResponse.json(await createSupportSession(principal, organisationId, await request.json()), { status: 201 });
  } catch (error) {
