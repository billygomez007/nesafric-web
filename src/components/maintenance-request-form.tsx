...
export function MaintenanceRequestForm() {
  ...
  return <form className="grid gap-5 rounded-2xl border bg-white p-6 shadow-sm" onSubmit={submit}>
    ...
    {error && <p className="rounded-lg bg-red-50 p-3 text-sm text-red-800">{error}</p>}<button className="rounded-lg bg-slate-950 p-3 font-semibold text-white disabled:opacity-50" disabled={saving || !propertyId}>{saving ? "Reporting..." : "Report maintenance issue"}</button>
  </form>;
}
