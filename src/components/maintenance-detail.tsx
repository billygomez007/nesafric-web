...
function ApprovalRequestForm({ currency, busy, onSubmit }: { currency: string; busy: boolean; onSubmit: (body: unknown) => void }) { return <form className="grid gap-2 rounded-lg border p-3" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); onSubmit({ requestedAmountMinor: data.get("amount"), currencyCode: currency, reason: data.get("reason") || undefined }); }}><p className="text-sm font-semibold">Request cost approval</p><input className="rounded-lg border p-2 text-sm" min={0} name="amount" placeholder={`Amount in minor units (${currency})`} required type="number" /><input className="rounded-lg border p-2 text-sm" name="reason" placeholder="Reason (optional)" /><button className="rounded-lg border p-2 text-sm font-semibold" disabled={busy}>Request approval</button></form>; }
...
function CostForm({ label, disabled, onSubmit }: { label: string; disabled: boolean; onSubmit: (amountMinor: string) => void }) { return <form className="flex gap-2" onSubmit={(event) => { event.preventDefault(); onSubmit(String(new FormData(event.currentTarget).get("amountMinor"))); }}><input className="min-w-0 flex-1 rounded-lg border p-2 text-sm" min={0} name="amountMinor" placeholder="Amount in minor units" required type="number" /><button className="rounded-lg border px-3 text-sm font-semibold" disabled={disabled}>{label}</button></form>; }
