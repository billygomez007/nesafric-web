...
const STATUS_COPY: Record<string, string> = {
  ...
  PENDING_APPROVAL: "Submitted — awaiting NesAfric review",
  APPROVED: "Approved",
  SCHEDULED: "Scheduled",
  ACTIVE: "Live",
  PAUSED: "Paused",
...
export function MarketplacePromotions({ professionalId }: { professionalId: string }) {
  ...
  async function submit(campaignId: string) {
    ...
    setNotice("Submitted for NesAfric review."); await load();
  }
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="rounded-xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">Requests are reviewed by NesAfric before appearing publicly. This is request/submission readiness — not yet a live self-service ad purchase.</p>
          ...
          <ul className="mt-3 divide-y">
            {campaigns.map((campaign) => (
              <li className="grid gap-1.5 py-3" key={campaign.id}>
                ...
                {campaign.status === "ACTIVE" && <p className="text-xs text-slate-400">{campaign.impressionCount} impressions · {campaign.clickCount} clicks</p>}
