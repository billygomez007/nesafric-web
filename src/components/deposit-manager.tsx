...
export function DepositManager() {
  ...
  return <div className="grid gap-6">
    <form className="grid gap-4 rounded-2xl border bg-white p-6 shadow-sm" onSubmit={create}><h2 className="text-xl font-semibold">Record security/caution deposit</h2><div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-medium">Lease<select className="mt-1 w-full rounded-lg border p-3" required value={leaseId} onChange={(event) => setLeaseId(event.target.value)}><option value="">Select lease</option>{leases.map((lease) => <option key={lease.id} value={lease.id}>{lease.referenceNumber}</option>)}</select></label><label className="text-sm font-medium">Tenant<select className="mt-1 w-full rounded-lg border p-3" disabled={!selectedLease} name="tenantOrganisationId" required><option value="">Select tenant</option>{selectedLease?.parties.map(({ tenantOrganisation }) => <option key={tenantOrganisation.id} value={tenantOrganisation.id}>{tenantOrganisation.tenant.preferredName || tenantOrganisation.tenant.legalName}</option>)}</select></label></div><div className="grid gap-4 md:grid-cols-3"><label className="text-sm font-medium">Amount<input className="mt-1 w-full rounded-lg border p-3" min="0.01" name="amount" required step="0.01" type="number" /></label><label className="text-sm font-medium">Received date<input className="mt-1 w-full rounded-lg border p-3" name="receivedAt" required type="datetime-local" /></label><label className="text-sm font-medium">Method<select className="mt-1 w-full rounded-lg border p-3" name="method"><option value="CASH">Cash</option><option value="BANK_TRANSFER">Bank transfer</option><option value="MOBILE_MONEY">Mobile Money</option></select></label></div><div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-medium">Reference<input className="mt-1 w-full rounded-lg border p-3" name="externalReference" required /></label><label className="text-sm font-medium">Evidence reference<input className="mt-1 w-full rounded-lg border p-3" name="evidenceReference" /></label></div><label className="text-sm font-medium">Notes<textarea className="mt-1 w-full rounded-lg border p-3" name="notes" rows={2} /></label><button className="rounded-lg bg-slate-950 p-3 font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-600" disabled={!selectedLease}>Record deposit</button>{success && <p className="rounded-lg bg-emerald-50 p-3 text-sm text-emerald-800">{success}</p>}{error && <p className="rounded-lg bg-red-50 p-3 text-sm text-red-800">{error}</p>}</form>
